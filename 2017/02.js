//Part1
function test(){
    const sheet = [[5, 1, 9, 5], [7, 5, 3], [2, 4, 6, 8]]
    console.log(checkSum(sheet) === 18)
}

//FP oh yeah
const sum = (a, b) => a + b
const maxLessMin = row => Math.max(...row) - Math.min(...row)

const checkSum = (matrix) => matrix.map(maxLessMin).reduce(sum)
